...
from feature_analysis_utils import scatter_plot_latent_separation_scores_experiment
...
top_k = 5
tokens_to_cache = 'entity'
top_scores_layers, minmax_layerwise_scores = get_layerwise_latent_scores(model_alias, LAYERS_WITH_SAE, tokens_to_cache, scoring_method,
                                                                         ALL_ENTITY_TYPES, top_k, feature_type=feature_type)

# %%
for known_label in ['known', 'unknown']:
    plot_layerwise_latent_scores(model_alias, LAYERS_WITH_SAE, top_scores_layers, minmax_layerwise_scores,
                                 known_label, top_k, feature_type=feature_type)
