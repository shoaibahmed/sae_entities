# copied from https://github.com/neelnanda-io/neel-plotly/blob/main/neel_plotly/plot.py
...
def imshow(tensor, renderer=None, **kwargs):
    ...
    fig = px.imshow(tensor, color_continuous_midpoint=0.0, **kwargs_pre)
    ...
    # font
    # kwargs_post["font_family"] = "Serif"
    # kwargs_post["font_size"] = 14
...
def plot_bar_chart(data_dict: Dict[str, Dict[str, float]], y: List[str], title: str=None, width: int=800, height: int=500, save_path=None, **kwargs):
    ...
    fig = px.bar(
        ...
        x="label",
        y=y, 
        title=title if title else "Accuracy",
    ...
    # Update layout
    layout_kwargs = {
        'xaxis_title': kwargs.get('xaxis_title', 'Label'),
        'yaxis_title': kwargs.get('yaxis_title', 'Count'),
        'legend_title': kwargs.get('legend_title', 'Result'),
        'font': kwargs.get('font', dict(size=14)),  # Increase font size
        'bargap': kwargs.get('bargap', 0.2)  # Adjust gap between bars
    }
...
def create_boxplot(data_dict):
    ...
    # Create a color map based on the first word of each key
    color_map = {}
    colors = plt.get_cmap('tab10')
    for i, key in enumerate(keys):
        first_word = key.split('_')[0]
        if first_word not in color_map:
            color_map[first_word] = colors(len(color_map) % 10)

    # Create the boxplot
    fig, ax = plt.subplots(figsize=(12, 6))
    bp = ax.boxplot(data, patch_artist=True)

    # Color the boxes based on the first word of each key
    for patch, key in zip(bp['boxes'], keys):
        first_word = key.split('_')[0]
        patch.set_facecolor(color_map[first_word])
    ...
    # Add a legend
    legend_elements = [plt.Rectangle((0, 0), 1, 1, fc=color, ec="k", label=word)
                       for word, color in color_map.items()]
    ax.legend(handles=legend_elements, title="First Word", loc="upper right")
